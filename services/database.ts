...
class DatabaseService {
  ...
  checkFirebaseConfig(): boolean {
    ...
    try {
      ...
      return Boolean(hasConfig) && typeof window !== "undefined";
    } catch (error) {
  ...
  async addProduct(productData: Partial<Product>): Promise<Product> {
    if (!this.useFirebase) {
      ...
      return { id: "mock_" + Date.now(), ...productData } as Product;
    }
  ...
  // Mock data methods
  getMockProducts(filters: ProductFilters = {}): Promise<Product[]> {
    let products: any[] = [...mockProducts];

    if (filters.category) {
      products = products.filter(
        (p: any) => p.category.toLowerCase() === filters.category?.toLowerCase()
      );
    }
  ...
  getMockProduct(id: string): Promise<Product | null> {
    const product = mockProducts.find((p: any) => p.id == id);
    return Promise.resolve((product as Product) || null);
  }

  getMockCategories(): Promise<any[]> {
    const categories: any[] = [
      {
        id: 1,
        name: "Electronics",
  ...
  getMockOrders(userId: string): Promise<any[]> {
    const orders = [
      {
        id: "order_1",
        userId,
