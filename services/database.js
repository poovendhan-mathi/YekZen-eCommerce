...
class DatabaseService {
  ...
  getMockProducts(filters = {}) {
    ...
    if (filters.orderBy === 'price') {
      products.sort((a, b) => filters.order === 'desc' ? b.price - a.price : a.price - b.price);
    } else if (filters.orderBy === 'name') {
      products.sort((a, b) => filters.order === 'desc' ? b.name.localeCompare(a.name) : a.name.localeCompare(b.name));
    }
  ...
  getMockCategories() {
    const categories = [
      { id: 1, name: 'Electronics', description: 'Latest gadgets and devices', count: 156 },
      { id: 2, name: 'Fashion', description: 'Trendy clothing and accessories', count: 234 },
      { id: 3, name: 'Home & Garden', description: 'Everything for your home', count: 89 },
      { id: 4, name: 'Sports', description: 'Sports equipment and fitness gear', count: 67 },
      { id: 5, name: 'Books', description: 'Wide selection of books', count: 345 },
      { id: 6, name: 'Beauty', description: 'Cosmetics and personal care', count: 123 }
    ];
  ...
  async initializeSampleData() {
    if (!this.useFirebase) {
      console.log('Firebase not configured, skipping sample data initialization');
      return;
    }
